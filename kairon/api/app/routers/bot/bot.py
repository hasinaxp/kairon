...
async def get_all_mail_configs(
    ...
    Fetches mail config
    """
    return {"data": {"mail-config": MailClassificationConfig.get_doc(current_user.get_bot())}}

@router.delete("/mail/config/{intent}", response_model=Response)
async def get_single_mail_config(
        intent: str,
        current_user: User = Security(Authentication.get_current_user_and_bot, scopes=DESIGNER_ACCESS)
):
    """
    get the mail config by intent
    """
    return {"data": {"mail-config": MailClassificationConfig.get_doc(current_user.get_bot(), intent)}}
...
async def set_mail_config(
    ...
    """
    request_dict = request_data.dict()
    MailClassificationConfig.update_doc(**request_dict, bot=current_user.get_bot(), user=current_user.get_user())
    return {"message": "Config applied!"}
