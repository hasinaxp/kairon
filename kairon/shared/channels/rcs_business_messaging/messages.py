...
class MessageCluster(object):
    ...
    """
    def __init__(self, rcs_service: rbm_service.RCSService):
        """
        Constructor.
        """
        self._messages = []
        self._suggestions = []
        self.rcs_service = rcs_service
    ...
    def send_to_msisdn(self, rmb_service_instance: rbm_service.RCSService, msisdn, timeToLive=None, expireTime=None):
        """
        Sends all the messages in the clsuter to the given MSISDN.
        
        Args:
        rmb_service_instance (rbm_service.RCSService): instance of RCSService
        ...
        for message in self._messages:
            message.prepare_to_send(rmb_service_instance)
        ...
        for i in range(0, len(self._messages)):
            ...
            if i < len(self._messages) - 1:
                resp = rmb_service_instance.send_message_with_body(msisdn,
                                                   message.get_agent_message(),
                                                   str(uuid.uuid4().int) + "a",
                                                   timeToLive, expireTime)
            else:
                logger.debug(message.get_agent_message())
                resp = rmb_service_instance.send_message_with_body_and_suggestion_chip_list(
                    msisdn,
                    message.get_agent_message(),
...
class Message(object):
    ...
    def prepare_to_send(self, rcs_service_instance: rbm_service.RCSService):
        """
...
class StandaloneCard(Message):
    ...
    def prepare_to_send(self, rbm_service_instance: rbm_service.RCSService):
        """
        rbm_service_instance: rcs_business_messaging.rbm_service.RCSService
        ...
        if not self._file_resource_id and self._url:
              self._file_resource_id = rbm_service_instance.upload_file(self._url, self._thumbnail_url)
...
class CarouselCard(Message):
    ...
    def prepare_to_send(self, rbm_service_instance: rbm_service.RCSService):
        ...
        """
        for card_content in self._card_contents:
            card_content.prepare_to_send(rbm_service_instance)
...
class CardContent(object):
    ...
    def get_card_content_object(self):
        ...
        # logger.debug("self._image_url: " + self._image_url)
