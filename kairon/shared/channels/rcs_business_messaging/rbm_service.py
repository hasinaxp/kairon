...
# https://github.com/rcs-business-messaging/rbm-api-examples/blob/master/python/rbm-api-helper/rcs_business_messaging/rbm_service.py
class RCSService:
    def __init__(self, certificate_json_str: str):
        self.cert_json = certificate_json_str
        self.cert_file = json.loads(self.cert_json)
        self.credentials = ServiceAccountCredentials.from_json_keyfile_dict(self.cert_file, scopes=scopes)
    ...
    def send_message_with_body(self, msisdn, body, message_id, timeToLive=None, expireTime=None):
        """
        ...
        body_string = self.emojize(json.dumps(body))
    
        resp, content = self.http_auth.request(endpoint_url,
                                          method='POST',
                                          body=body_string,
    ...
    def send_event_with_body(self, msisdn, body, message_id):
        """
        ...
        logger.info('send_event_with_body()\n\tendpoint_url: ' +
                        endpoint_url +
                        '\n\tpost_body: ' +
                        body_string +
                        '\n\tresp status: ' +
    ...
    def send_message_with_body_and_suggestion_chip_list(self, msisdn, body, suggestion_chip_list, message_id, timeToLive=None,
                                                        expireTime=None):
        """
        ...
        return self.send_message_with_body(msisdn, body, message_id, timeToLive, expireTime)
    ...
    def make_cap_request(self, msisdn):
        """
    ...
    def make_batch_cap_request(self, msisdns):
        """
        ...
        logger.info('make_batch_cap_request()\n\tendpoint_url: ' +
                        endpoint_url +
                        '\n\tresp status: ' +
                        str(resp.status) +
                        '\n\tresp reason: ' +
    ...
    def revoke(self, msisdn, message_id):
        """
        ...
        resp, content = self.http_auth.request(endpoint_url, method="DELETE")
    ...
    def invite_tester(self,msisdn):
        ...
        resp, content = self.http_auth.request(endpoint_url, method='POST')
    
        logger.info('invite_tester()\n\tendpoint_url: ' +
                        endpoint_url +
                        '\n\tresp status: ' +
                        str(resp.status) +
                        '\n\tresp reason: ' +
    ...
    def get_user_response_text(self, user_event):
        """
    ...
    def send_read_for_user_event_and_is_typing_if_message(self, user_event):
        """
        ...
        if self.get_user_response_text(user_event) and 'messageId' in user_event:
            agent_event = {
                'eventType': 'READ',
                'messageId': user_event['messageId']
            }
            self.send_event_with_body(user_event['senderPhoneNumber'],
                                 agent_event,
                                 str(uuid.uuid4().int) + "a")
            self.send_is_typing_event(user_event['senderPhoneNumber'])

    def send_is_typing_event(self,msisdn):
        """
        ...
        self.send_event_with_body(msisdn, agent_event, str(uuid.uuid4().int) + "a")